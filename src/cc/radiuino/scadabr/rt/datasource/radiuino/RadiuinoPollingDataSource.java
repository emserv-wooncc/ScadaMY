		...
		try {
			...
			for (DataPointRT dataPoint : dataPoints) {
				RadiuinoPointLocatorVO dataPointVO = dataPoint.getVO()
						.getPointLocator();
				if (!listSensores.contains(dataPointVO.getEnderecoSensor())) {
					listSensores.add(dataPointVO.getEnderecoSensor());
				}
			}

			LOG.debug("Montar a lista distinta dos sensores para fazer somente uma vez.");

			// Enviar os pacotes para cada sensor e fazer o parse da resposta
			for (Integer enderecoSensor : listSensores) {
				...
				for (DataPointRT dataPoint : dataPoints) {
					RadiuinoPointLocatorVO dataPointVO = dataPoint.getVO()
							.getPointLocator();
					if (enderecoSensor.equals(dataPointVO.getEnderecoSensor())) {
						// Sleep mode
						...
						// }
						String mapaBytes = dataPointVO.getMapaBytesRequisicao();
						for (int i = 0; i < 52; i++) {
				...
				LOG.debug("Preenchendo o endereco do sensor de destino.");

				pacoteEnvio[INDICE_ENDERECO_DESTINO] = enderecoSensor
						.byteValue();

				try {

					LOG.debug("Enviando o pacote na porta serial.");

					enviarPacote(pacoteEnvio, 1);

				} catch (IOException io) {
					LOG.error("Erro ao enviar pacote.", io);
					getsPort().close();
					reconnect();
				}
			}
			...
			LOG.error("Erro ao realizar o doPoll.", e);
		}
	}
		...
		case SerialPortEvent.DATA_AVAILABLE:
			if (esperandoPacote) {
				...
				try {
					...
					if (iBuffer == 52) {
						if (pacote[INDICE_ENDERECO_ORIGEM] == esperandoPacoteEndereco) {
							esperandoPacote = false;
						}
