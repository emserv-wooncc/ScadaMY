...
public class AuditEventType extends EventType {
	...
	// /
	//
	private static Log log = LogFactory.getLog(AuditEventType.class);
	...
	static {
		...
		TYPE_CODES
				.addElement(TYPE_POINT_EVENT_DETECTOR, "POINT_EVENT_DETECTOR");
		TYPE_CODES.addElement(TYPE_COMPOUND_EVENT_DETECTOR,
				"COMPOUND_EVENT_DETECTOR");
	...
	public static List<EventTypeVO> getAuditEventTypes() {
		if (auditEventTypes == null) {
			...
			addEventTypeVO(TYPE_POINT_EVENT_DETECTOR,
					"event.audit.pointEventDetector");
			addEventTypeVO(TYPE_COMPOUND_EVENT_DETECTOR,
					"event.audit.compoundEventDetector");
			...
			addEventTypeVO(TYPE_MAINTENANCE_EVENT,
					"event.audit.maintenanceEvent");
		}
	...
	private static void addEventTypeVO(int type, String key) {
		auditEventTypes
				.add(new EventTypeVO(EventType.EventSources.AUDIT, type, 0,
						new LocalizableMessage(key), SystemSettingsDao
								.getIntValue(AUDIT_SETTINGS_PREFIX + type,
	...
	public static void raiseAddedEvent(int auditEventTypeId,
			ChangeComparable<?> o) {
	...
	public static <T> void raiseChangedEvent(int auditEventTypeId, T from,
			ChangeComparable<T> to) {
		List<LocalizableMessage> changes = new ArrayList<LocalizableMessage>();
		log.debug("Raise Audit Event!");
		to.addPropertyChanges(changes, from);
		if (changes.size() == 0)
			// If the object wasn't in fact changed, don't raise an event.
			return;
		raiseEvent(auditEventTypeId, to, "event.audit.changed",
				changes.toArray());
	}

	public static void raiseDeletedEvent(int auditEventTypeId,
			ChangeComparable<?> o) {
		List<LocalizableMessage> list = new ArrayList<LocalizableMessage>();
		o.addProperties(list);
	...
	private static void raiseEvent(int auditEventTypeId, ChangeComparable<?> o,
			String key, Object[] props) {
		...
		LocalizableMessage message = new LocalizableMessage(key, username,
				new LocalizableMessage(o.getTypeKey()), o.getId(),
				new LocalizableMessage("event.audit.propertyList."
						+ props.length, props));

		AuditEventType type = new AuditEventType(auditEventTypeId, o.getId());
		type.setRaisingUser(user);

		Common.ctx.getEventManager().raiseEvent(type,
				System.currentTimeMillis(), false,
				getEventType(type.getAuditEventTypeId()).getAlarmLevel(),
				message, null);
	}
	...
	// /
	//
	public static void addPropertyMessage(List<LocalizableMessage> list,
			String propertyNameKey, Object propertyValue) {
		list.add(new LocalizableMessage("event.audit.property",
				new LocalizableMessage(propertyNameKey), propertyValue));
	}

	public static void addPropertyMessage(List<LocalizableMessage> list,
			String propertyNameKey, boolean propertyValue) {
		list.add(new LocalizableMessage("event.audit.property",
				new LocalizableMessage(propertyNameKey),
				getBooleanMessage(propertyValue)));
	}

	public static void addPeriodMessage(List<LocalizableMessage> list,
			String propertyNameKey, int periodType, int period) {
		list.add(new LocalizableMessage("event.audit.property",
				new LocalizableMessage(propertyNameKey), Common
						.getPeriodDescription(periodType, period)));
	}

	public static void addExportCodeMessage(List<LocalizableMessage> list,
			String propertyNameKey, ExportCodes codes, int id) {
		list.add(new LocalizableMessage("event.audit.property",
				new LocalizableMessage(propertyNameKey), getExportCodeMessage(
						codes, id)));
	}

	public static void addDataTypeMessage(List<LocalizableMessage> list,
			String propertyNameKey, int dataTypeId) {
		list.add(new LocalizableMessage("event.audit.property",
				new LocalizableMessage(propertyNameKey), DataTypes
						.getDataTypeMessage(dataTypeId)));
	}

	public static void maybeAddPropertyChangeMessage(
			List<LocalizableMessage> list, String propertyNameKey,
	...
	public static void maybeAddPropertyChangeMessage(
		...
		if (fromValue != toValue)
			addPropertyChangeMessage(list, propertyNameKey,
					getBooleanMessage(fromValue), getBooleanMessage(toValue));
	}

	public static void maybeAddAlarmLevelChangeMessage(
			List<LocalizableMessage> list, String propertyNameKey,
			int fromAlarmLevel, int toAlarmLevel) {
		if (fromAlarmLevel != toAlarmLevel)
			addPropertyChangeMessage(list, propertyNameKey,
					AlarmLevels.getAlarmLevelMessage(fromAlarmLevel),
					AlarmLevels.getAlarmLevelMessage(toAlarmLevel));
	}

	public static void maybeAddPeriodChangeMessage(
			List<LocalizableMessage> list, String propertyNameKey,
			int fromPeriodType, int fromPeriod, int toPeriodType, int toPeriod) {
		if (fromPeriodType != toPeriodType || fromPeriod != toPeriod)
			addPropertyChangeMessage(list, propertyNameKey,
					Common.getPeriodDescription(fromPeriodType, fromPeriod),
					Common.getPeriodDescription(toPeriodType, toPeriod));
	}

	public static void maybeAddExportCodeChangeMessage(
			List<LocalizableMessage> list, String propertyNameKey,
			ExportCodes exportCodes, int fromId, int toId) {
		if (fromId != toId)
			addPropertyChangeMessage(list, propertyNameKey,
					getExportCodeMessage(exportCodes, fromId),
					getExportCodeMessage(exportCodes, toId));
	}

	public static void maybeAddDataTypeChangeMessage(
			List<LocalizableMessage> list, String propertyNameKey,
			int fromDataTypeId, int toDataTypeId) {
		if (fromDataTypeId != toDataTypeId)
			addPropertyChangeMessage(list, propertyNameKey,
					DataTypes.getDataTypeMessage(fromDataTypeId),
	...
	private static LocalizableMessage getExportCodeMessage(
			ExportCodes exportCodes, int id) {
	...
	public static void addPropertyChangeMessage(List<LocalizableMessage> list,
			String propertyNameKey, Object fromValue, Object toValue) {
		list.add(new LocalizableMessage("event.audit.changedProperty",
				new LocalizableMessage(propertyNameKey), fromValue, toValue));
	}
	...
	public String toString() {
		return "AuditEventType(auditTypeId=" + auditEventTypeId
